...
class DeapEegX:
    ...
    def process_raw_data(self):
        ...
        for user in range(
            self.cf["nUser_train_head"], self.cf["nUser_train_end"] + 1, self.cf["nUser_train_step"]
        ):  # nUser
            filename = self.cf["raw_data_eeg_path"] + "s" + f"{user:02d}" + ".dat"
            ...
            for tr in range(
                self.cf["nTrial_train_head"] - 1,
                self.cf["nTrial_train_end"],
                self.cf["nTrial_train_step"],
            ):  # nTrial
                for ch in self.cf["nChannel_ON"]:
                    for dat in range(
                        self.cf["nTime_head"], self.cf["nTime_end"]
                    ):  # nTime interval : 384 - 8064, total =  60 seconds
                        train_data_eeg.write(
                            str(x["data"][tr][self.cf["nChannel_ON"][ch]][dat]) + " "
                        )
        ...
        for user in range(
            self.cf["nUser_test_head"], self.cf["nUser_test_end"] + 1, self.cf["nUser_test_step"]
        ):  # nUser
            filename = self.cf["raw_data_eeg_path"] + "s" + f"{user:02d}" + ".dat"
            ...
            for tr in range(
                self.cf["nTrial_test_head"] - 1,
                self.cf["nTrial_test_end"],
                self.cf["nTrial_test_step"],
            ):  # nTrial
                for ch in self.cf["nChannel_ON"]:
                    for dat in range(
                        self.cf["nTime_head"], self.cf["nTime_end"]
    ...
    def add_class_label(self):
        ...
        with open(self.cf["train_label_valance_path"]) as file:
            for value in file:
        ...
        with open(self.cf["train_label_arousal_path"]) as file:
            for value in file:
                if float(value) > self.cf["label_threshold"]:  # default 4.5
        ...
        with open(self.cf["test_label_valance_path"]) as file:
            for value in file:
                if float(value) > self.cf["label_threshold"]:  # default 4.5
        ...
        with open(self.cf["test_label_arousal_path"]) as file:
            for value in file:
                if float(value) > self.cf["label_threshold"]:  # default 4.5
    ...
    def load_dataset(self):
        ...
        self.train_label_valance = numpy.genfromtxt(
            self.cf["train_label_valance_class_path"], delimiter=" "
        )
        self.train_label_arousal = numpy.genfromtxt(
            self.cf["train_label_arousal_class_path"], delimiter=" "
        )
        # *********************************************************************************************
        # LOAD TEST SET
        # *********************************************************************************************
        self.test_data = numpy.genfromtxt(self.cf["test_data_eeg_path"], delimiter=" ")
        self.test_label_valance = numpy.genfromtxt(
            self.cf["test_label_valance_class_path"], delimiter=" "
        )
        self.test_label_arousal = numpy.genfromtxt(
            self.cf["test_label_arousal_class_path"], delimiter=" "
        )
    ...
    def model_result(self):
        self.confusion_matrix_arousal = confusion_matrix(
            self.test_label_arousal, self.predict_arousal
        )
        self.confusion_matrix_valance = confusion_matrix(
            self.test_label_valance, self.predict_valance
        )
        print(self.confusion_matrix_arousal)
        print(self.confusion_matrix_valance)
        print(
            "Arousal accuracy score = "
            + str(accuracy_score(self.test_label_arousal, self.predict_arousal))
        )
        print(
            "Valance accuracy score = "
            + str(accuracy_score(self.test_label_valance, self.predict_valance))
        )
